...
grup AS (
  ...
  FROM hstgrp
  WHERE name LIKE '%TBN%'
    AND name NOT REGEXP 'SAP|01_Critical|02_Very High|03_High|04_Medium|05_Low'
),
...
list_server AS (
  ...
  WHERE h.name NOT LIKE 'tbn-com-hp%'
    ...
    AND NOT EXISTS (
      ...
      JOIN hstgrp gg2 ON gg2.groupid = hg2.groupid
      WHERE hg2.hostid = h.hostid
        AND gg2.name REGEXP 'SAP|01_Critical|02_Very High|03_High|04_Medium|05_Low'
    )
),
